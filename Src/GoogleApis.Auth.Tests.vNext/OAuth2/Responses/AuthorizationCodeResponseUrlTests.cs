...
namespace Google.Apis.Auth.OAuth2.Responses
{
    ...
    public class AuthorizationCodeResponseUrlTests
    {
        ...
        {
            Assert.Equal(response.Code, Is.EqualTo("123"));
            Assert.Equal(response.Error, Is.EqualTo("ERR"));
            Assert.Equal(response.ErrorUri, Is.EqualTo("URI"));
            Assert.Equal(response.ErrorDescription, Is.EqualTo("DESC"));
            Assert.Equal(response.State, Is.EqualTo("STATE"));
        }
    }
}
