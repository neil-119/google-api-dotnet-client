    ...
    class MaxUrlLengthInterceptorTest
    {
        ...
        private void SubtestGetWithUrlOkayLengthByDefault(uint length, HttpMethod method)
        {
            ...
            using (var httpClient = new HttpClient(handler))
            {
                ...
                Assert.Equal((long)request.RequestUri.ToString().Length, (long)Is.EqualTo(length));
                Assert.Equal(request.Method, Is.EqualTo(method));
        ...
        public void TestGetPathQueryWithUrlOkayLengthByDefault()
        {
            ...
            using (var httpClient = new HttpClient(handler))
            {
                ...
                Assert.Equal(mockHandler.RequestContent, Is.EqualTo(query));
            }
        }
        ...
        public void TestGetWithUrlTooLongByUserSpecified()
        {
            ...
            using (var httpClient = new HttpClient(handler))
            {
                ...
                Assert.Equal((long)requestUri.Length, (long)Is.EqualTo(maxUrlLength + 1));
