...
namespace Google.Apis.Tests.Apis.Requests
{
    ...
    {
        public ClientServiceRequestTest()
        {
        ...
        class TestBodyMessageHandler : CountableMessageHandler
        {
                ...
                CancellationToken cancellationToken)
            {
                ...
                if (GZipEnabled)
                {
                    ...
                    Assert.True(encoding.Count == 1 && encoding.First().Equals("gzip"), "Content-Encoding should be 'gzip'");
                ...
                Assert.Equal(request.Content.Headers.ContentType, Is.EqualTo(new MediaTypeHeaderValue(mediaType)
                    {
                ...
                Assert.Equal(obj, Is.EqualTo(ExpectedRequestObject));
        ...
        private void SubtestExecute_GZip(bool gzip, bool async)
        {
            ...
            using (var service = new MockClientService(initializer))
            {
                ...
                Assert.Equal(response, Is.EqualTo(handler.ResponseObject));
            }
        }
        ...
        private void SubtestExecute_ThrowException(bool backOff)
        {
            ...
            using (var service = new MockClientService(initializer))
            {
                ...
                Assert.Equal(handler.Calls, Is.EqualTo(calls));
            }
        }
        ...
        private void SubtestExecuteAsync_ThrowException(bool backOff)
        {
            ...
            using (var service = new MockClientService(initializer))
            {
                ...
                {
                    Assert.IsAssignableFrom(typeof(InvalidOperationMockException), ex.InnerException);
                }
        ...
        public void ExecuteAsync_Simultaneously()
        {
            ...
            using (var service = new MockClientService(initializer))
            {
                ...
                for (var i = 1; i <= tasks.Count; ++i)
                {
                    ...
                    Assert.Equal(response.Name, Is.EqualTo("Name-" + nameIndex));
                }

                // half of the request should succeed in the second call, so total calls should be calls + calls/2
                Assert.Equal(handler.Calls, Is.EqualTo(calls + calls / 2));
            }
        }
        ...
        public void CreateRequest_QueryParameters()
        {
            using (var service = new MockClientService("https://build_request_params"))
            {
                ...
                Assert.Equal(httpRequest.RequestUri, Is.EqualTo(new Uri(
        ...
        public void CreateRequest_QueryDateParameter()
        {
            using (var service = new MockClientService("https://build_request_params"))
            {
                ...
                var httpRequest = request.CreateRequest();
                Assert.Equal(httpRequest.RequestUri, Is.EqualTo(new Uri("https://build_request_params/restPath0")));

                // Request with the date parameter.
                DateTime date = new DateTime(2002, 2, 25, 12, 57, 32, 777, DateTimeKind.Utc);
                request.Min = date;
                httpRequest = request.CreateRequest();
                Assert.Equal(httpRequest.RequestUri, Is.EqualTo(
        ...
        public void CreateRequest_ETag()
        {
            ...
            using (var service = new MockClientService())
            {
                ...
                var httpRequest = request.CreateRequest();
                Assert.Equal(httpRequest.Headers.IfMatch.Count, Is.EqualTo(0));
                Assert.Equal(httpRequest.Headers.IfNoneMatch.Count, Is.EqualTo(0));
                ...
                httpRequest = request.CreateRequest();
                Assert.Equal(httpRequest.Headers.IfMatch.Count, Is.EqualTo(0));
                Assert.Equal(httpRequest.Headers.IfNoneMatch.Count, Is.EqualTo(1));
                Assert.Equal(httpRequest.Headers.IfNoneMatch.First(), Is.EqualTo(new EntityTagHeaderValue(body.ETag)));

                // ETag has a value, so use default action (Post -> If-Match).
                request = new TestClientServiceRequest(service, HttpConsts.Post, body);
                httpRequest = request.CreateRequest();
                Assert.Equal(httpRequest.Headers.IfNoneMatch.Count, Is.EqualTo(0));
                Assert.Equal(httpRequest.Headers.IfMatch.Count, Is.EqualTo(1));
                Assert.Equal(httpRequest.Headers.IfMatch.First(), Is.EqualTo(new EntityTagHeaderValue(body.ETag)));
